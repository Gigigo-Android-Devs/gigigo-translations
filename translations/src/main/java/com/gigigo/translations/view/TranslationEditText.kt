...
open class TranslationEditText : EditText {
  ...
  constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(context, attrs,
    ...
    val a = getContext().theme.obtainStyledAttributes(attrs, R.styleable.TranslationEditText, 0, 0)

    try {
      val translationKey = a.getResourceId(R.styleable.TranslationEditText_translation_hint, -1)
      if (translationKey != -1) {
        val text = translationsManager.getTextFromKey(translationKey)
        hint = formatText(text)
      }
    } finally {
      a.recycle()
    }
